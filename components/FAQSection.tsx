...
const faqData: FAQItem[] = [
  ...
  {
    id: 6,
    category: "Your Answer Share™ Score",
  ...
  {
    id: 10,
    category: "Improving Your Visibility",
  ...
  {
    id: 19,
    category: "Working with Sarah / Zen Media",
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-300 text-lg">Everything you need to know about AI visibility</p>
        </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-all ${
              selectedCategory === null
                ...
                : 'bg-brand-surface border border-brand-border text-brand-text hover:border-brand-cyan'
            }`}
          >
            All Questions
          </button>
        ...
        <div className="space-y-4">
          {filteredFAQ.map(item => (
            <motion.div
              ...
              className="border border-brand-border rounded-lg bg-brand-dark/50 hover:border-brand-cyan/50 transition-colors"
            >
              <button
                onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}
                className="w-full p-6 flex items-start justify-between text-left hover:bg-brand-surface/30 transition-colors"
              >
                <div className="flex-1">
                  <p className="text-sm text-brand-cyan font-semibold mb-2">{item.category}</p>
                  <h3 className="text-lg font-semibold text-brand-text leading-snug">{item.question}</h3>
                </div>
                <ChevronDown
                  className={`w-5 h-5 text-brand-cyan ml-4 flex-shrink-0 transition-transform ${
        ...
        <div className="mt-16 p-8 bg-gradient-to-r from-brand-cyan/10 to-brand-blue/10 rounded-lg border border-brand-cyan/20 text-center">
          <h3 className="text-2xl font-bold mb-4">Still have questions?</h3>
          <p className="text-gray-300 mb-6">
            Reach out to the team at Zen Media and let's talk about your AI visibility strategy.
          </p>
          <a
            href="https://zenmedia.com/services/"
            className="inline-block px-8 py-3 bg-brand-cyan text-brand-dark font-bold rounded-lg hover:bg-brand-blue transition glow-cyan"
          >
            Get Started
          </a>
        </div>
